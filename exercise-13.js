...
function countProfit(shoppers) {
  ...
  for (var i = 0; i < listBarang.length ; i++){
    var objOutput = {};
    objOutput.product = listBarang[i][0];
    objOutput.shoppers = [];
    objOutput.leftOver = listBarang[i][2];
    for(var j = 0; j < shoppers.length; j++){
        if(shoppers[j].product == listBarang[i][0]){
            objOutput.shoppers.push(shoppers[j].name);
            if(shoppers[j].amount <= objOutput.leftOver){
                objOutput.leftOver -= shoppers[j].amount ;
            }
        }
    }
    objOutput.totalProfit = (listBarang[i][2] - objOutput.leftOver) * listBarang[i][1];
