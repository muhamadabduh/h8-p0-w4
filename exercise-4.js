...
function cariModus(arr) {
    ...
    // var newModus = modus;
    // for (var l = 0; l < newModus.length; l++){
    //     for(var m = 0; m < newModus.length-l-1; m++){
    //         if(newModus[m] > newModus[m+1]){
    //             var tmp = newModus[m];
    //             newModus[m] = newModus[m+1];
    //             newModus[m+1] = tmp;
    //         }
    //     }
    // }    
    ...
    //     tampung++
    // }
    // console.log('ini ' + tampung);
    // console.log(modus);

    for (var k = 0; k < modus.length; k++){
        if(modus[k] == 1){
            tampung++;
        } else if (modus[k] === modus.length){
            return -1;
        } else if(modus[k] === Math.max(...modus)){
            return arr[k];
        }
    }
    
    if(tampung == modus.length){
        return -1;
    }
}
